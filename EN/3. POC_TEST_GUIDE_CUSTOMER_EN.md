...
- Operational settings test:
  - In `/admin/settings`, update **minimum order total / default shipping fee / free-shipping threshold / low-stock threshold**
...
- Inventory operations test:
  - In `/admin/inventory`, review the low-stock report and export it as CSV.
...
- Order operations test:
  - In `/admin/orders`, run Order CSV export (with date filters) and confirm it exports one row per order item.
  - On an order detail page, set carrier/tracking number and change status to shipped/delivered to confirm shipped/delivered timestamps are recorded.
